      ...
      <div class="col-xl-12">
        <div class="card height-equal title-line">
          <div class="card-header">
            <div class="row">
                <div class="col-7">
                    <h2>Actaulizacao de Despesa</h2>
                    <p class="f-m-light mt-1">
                        Faça a Actaulização da Despesa
                    </p>
                </div> 
                <div class="col-5">
            ...
            <div class="card-body basic-wizard important-validation">
              ...
              <form class="row g-3 needs-validation custom-input" id="msform" enctype="multipart/form-data">
                  ...
                  <form1 class="stepper-one row g-3 needs-validation custom-input" >
                    ...
                    <input class="form-control"  type="text" name="id" value="{{$despesa->id}}"  hidden>
                        ...
                        <label class="form-label" for="passwordwizard">Categorias<span class="txt-danger">*</span></label>
                          <select class="form-select"  name="categoria" >
                                ...
                                @foreach($categorias as $categoria)
                                    <option value="{{$categoria->id}}" @if($despesa->categoria_id == $categoria->id) selected @endif>{{$categoria->nome}}</option>
                                @endforeach
                          </select>
                    </div>
                    ...
                    <div class="col-4">
                      ...
                      <input class="form-control"  type="text" name="valor_despesa"  value="{{$despesa->valor_despesa}}" >
                    </div>

                    <div class="col-4">
                      <label class="form-label" for="passwordwizard">Valor Pago<span class="txt-danger">*</span></label>
                      <input class="form-control"  type="text" name="valor_pago"  value="{{$despesa->valor_pago}}" >
                    </div>
                    ...
                    <div class="col-4">
                        <label class="form-label" for="passwordwizard">Estado Pagamento<span class="txt-danger">*</span></label>
                          <select class="form-select"  name="estado">
                                ...
                                <option value="Pago" @if($despesa->estado == 'Pago') selected @endif>Pago</option>
                                <option value="Completo" @if($despesa->estado == 'Completo') selected @endif>Completo</option>
                                <option value="Pago Parcial" @if($despesa->estado == 'Pago Parcial') selected @endif>Pago Parcial</option>
                                <option value="Pendente" @if($despesa->estado == 'Pendente') selected @endif>Pendente</option>
                          </select>
                    </div>

                    <div class="col-4">
                      <label class="form-label" for="passwordwizard">Data Pagamento<span class="txt-danger">*</span></label>
                      <input class="form-control"  type="date" name="data_pagamento" value="{{$despesa->data_pagamento}}" >
                    </div>

                  </form1>
                  <div class="wizard-footer d-flex gap-2 justify-content-end">
                      <button id="botao_salvar" type="submit" class="btn btn-success w-100">
                          <span id="botao_texto">{{__('Actualizar Despesa')}} </span>
        ...
        submitHandler: function(form) {
            $.ajax({
                ...
                beforeSend: function () {
                    ...
                    $('#botao_texto').text('Regsitando Despesa...');
                },
                ...
                success: function(response) {
                    ...
                    $('#botao_texto').text('Registar Despesa');
